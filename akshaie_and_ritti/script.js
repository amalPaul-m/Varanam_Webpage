...
function initCountdownTimer() {
  const targetDate = new Date('2026-09-20T11:30:00+05:30').getTime();
...
────────────────────────────────────────────────────────── */
const BLESSINGS_STORAGE_KEY = 'akshaie_ritti_engagement_wishes_v2';
const GOOGLE_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbz6gSdQqMg0Q6BaBcqIgDvxp8Feo4ce_D5MmAFhE-ZsHLYck6-ADyI8R-FtDthMu9Q/exec';
const GOOGLE_SHEET_JSON_URL = 'https://docs.google.com/spreadsheets/d/16J5iBHGlq48GbP5JWGRYQ5dbDwdPplbRlQErXY9_nL8/gviz/tq?tqx=out:json';
...
function addToCalendar() {
  const title = "Akshaie & Ritti's Engagement Ceremony";
  const details = "Engagement Ceremony & Reception. You are cordially invited to celebrate with us.";
  const location = "St Mary's Church Kalayanthani, Thodupuzha";
  const start = "20260920T003000Z";
  const end = "20260920T100000Z";
...
function addAppleCalendar() {
  ...
  const icsMSG = `BEGIN:VCALENDAR\nVERSION:2.0\nPRODID:-//Akshaie and Ritti Engagement//EN\nBEGIN:VEVENT\nUID:${new Date().getTime()}@akshaieandritti.com\nDTSTAMP:${new Date().toISOString().replace(/[-:]/g, '').split('.')[0] + 'Z'}\nDTSTART:${start}\nDTEND:${end}\nSUMMARY:${title}\nDESCRIPTION:${details}\nLOCATION:${location}\nEND:VEVENT\nEND:VCALENDAR`;

  const blob = new Blob([icsMSG], { type: 'text/calendar;charset=utf-8' });
  const url = window.URL.createObjectURL(blob);
  const link = document.createElement('a');
  link.href = url;
  link.setAttribute('download', 'akshaie_ritti_engagement.ics');
...
function initFloatingFlowers() {
  ...
  function spawnFlower(initial = false) {
    ...
    flower.style.top = initial ? `${Math.random() * 20}vh` : '-40px';
    ...
    flower.style.animation = `flowerFall ${duration}s linear infinite normal, flowerSway ${swayDuration}s ease-in-out infinite alternate`;
